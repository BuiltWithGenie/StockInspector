...
using YFinance, DataFrames, PlotlyJS, Statistics, Dates
...
function get_correlation(prices)
	df_pivot = dropmissing!(unstack(prices, :timestamp, :ticker, :close))
cor(Matrix(df_pivot[:, 2:end]))
end

# ╔═╡ be920cd2-cd8f-4b5c-85a9-5b9641f4b53d


# ╔═╡ cacbb325-cf97-4059-9b52-b39fd559442e
cor_matrix = get_correlation(prices)
...
function pct_change(df::AbstractDataFrame, col_name::Symbol)
    col = df[!, col_name]
    return [0; (col[2:end] .- col[1:end-1]) ./ col[1:end-1]]
end

# ╔═╡ 226d7635-0228-43da-bd92-93b24bf1ca2b
pct_change(prices, :close)

# ╔═╡ 030efd96-2dae-423c-970b-030acb101921
function get_monthly_prices!(prices::AbstractDataFrame)
	prices[!, :month] = Dates.lastdayofmonth.(prices.timestamp)
   monthly = combine(groupby(prices, [:month, :ticker]), 
                  [:close] => last => :close)
monthly[!, :returns] = vcat([pct_change(m, :close) for m in groupby(monthly,:ticker)]...)
monthly
end

# ╔═╡ 5673b954-1c39-4fda-9d0d-c10a84e7f781
...
period_diff = combine(groupby(monthly, :ticker)) do gdf
    close_diff = last(gdf.close) - first(gdf.close)
    percent_diff = (close_diff / first(gdf.close)) * 100
    return (total = close_diff, percent = percent_diff)
end
end

# ╔═╡ 602e53c1-8fd4-4a1d-be39-9e82656a1cfc
get_period_diff(monthly)

# ╔═╡ 998c1c22-08cb-43ef-b6a6-9118c9b43bbb
function get_news(symbols::Vector{String})
...
manifest_format = "2.0"
project_hash = "97f33e3d11cb9190c7b010e9e84cb6d0944e22fd"
